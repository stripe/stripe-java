...
public class StandardizationTest {
    @Test
    public void allNonDeprecatedMethodsHaveCorrespondingMethodWithRequestOptions() throws IOException, NoSuchMethodException {
        for (Class<?> model : getAllModels()) {
            for (Method method : model.getMethods()) {
                ...
                Class<?>[] extendedMethodParameterTypes = Arrays.copyOf(methodParameterTypes, methodParameterTypes.length + 1);
                extendedMethodParameterTypes[methodParameterTypes.length] = RequestOptions.class; // Add RequestOptions to the end
                Method correspondingMethod = getCorrespondingMethodWithRequestOptions(model, method, extendedMethodParameterTypes);
                assertNotNull(correspondingMethod,
                        String.format("Methods on %ss like %s %s should have a corresponding method with RequestOptions as the last argument.%n",
                                ApiResource.class.getSimpleName(),
                                method.getName(),
                                method.getParameterTypes().length > 0 ? "with parameters " + Arrays.toString(method.getParameterTypes()) : ""));
            }
        }
    }

    private boolean methodShouldBeSkipped(Method method, Class<?> model) {
        return method.getName().startsWith("set") || method.getName().startsWith("get") ||
                method.getName().startsWith("_") || method.getName().equals("equals") ||
                method.getDeclaringClass() != model || method.getName().equals("hashCode") || method.isAnnotationPresent(Deprecated.class) || getPrivacyStatus(method);
    }

    private Method getCorrespondingMethodWithRequestOptions(Class<?> model, Method method, Class<?>[] extendedMethodParameterTypes){
        try {
