  ...
  public void allNonDeprecatedMethodsTakeOptions() throws IOException, NoSuchMethodException {
    for (Class<?> model : getAllModels()) {
      for (Method method : model.getMethods()) {
        ...
        Method mostSpecificMethod = model.getDeclaredMethod(method.getName(), parameterTypes);
        ...
        // Check if an overload with RequestOptions as the last parameter exists
        if (!Arrays.stream(parameterTypes).anyMatch(p -> p.equals(RequestOptions.class))) {
          Class<?>[] overloadParameterTypes =
              Arrays.copyOf(parameterTypes, parameterTypes.length + 1);
          Array.set(
              overloadParameterTypes, overloadParameterTypes.length - 1, RequestOptions.class);
          Method overloadedMethod =
              model.getDeclaredMethod(method.getName(), overloadParameterTypes);
