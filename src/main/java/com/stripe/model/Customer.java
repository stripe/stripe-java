...
public class Customer extends ApiResource implements HasId, MetadataStore<Customer> {
  ...
  public Customer delete(Map<String, Object> params, RequestOptions options)
    ...
    String url =
        String.format(
            "%s/v1/customers/%s", Stripe.getApiBase(), ApiResource.urlEncodeId(this.getId()));
    return ApiResource.request(
  ...
  public static Customer retrieve(
    ...
    String url =
        String.format("%s/v1/customers/%s", Stripe.getApiBase(), ApiResource.urlEncodeId(customer));
