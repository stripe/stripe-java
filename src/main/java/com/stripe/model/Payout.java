...
public class Payout extends ApiResource implements MetadataStore<Payout>, BalanceTransactionSource {
  ...
  /**
   * Returns {@code true} if the payout was created by an <a href="https://stripe.com/docs/payouts#payout-schedule">automated payout schedule</a>, and {@code false} if it was <a href="https://stripe.com/docs/payouts#manual-payouts">requested manually</a>.
   */
  ...
  /**
   * Error code explaining reason for payout failure if available. See <a href="https://stripe.com/docs/api#payout_failures">Types of payout failures</a> for a list of failure codes.
   */
  ...
  /**
   * The method used to send this payout, which can be {@code standard} or {@code instant}. {@code instant} is only supported for payouts to debit cards. (See <a href="https://stripe.com/blog/instant-payouts-for-marketplaces">Instant payouts for marketplaces</a> for more information.)
   */
  ...
  /**
   * The source balance this payout came from. One of {@code card}, {@code fpx}, or {@code bank_account}.
   */
  ...
  /**
   * Current status of the payout: {@code paid}, {@code pending}, {@code in_transit}, {@code canceled} or {@code failed}. A payout is {@code pending} until it is submitted to the bank, when it becomes {@code in_transit}. The status then changes to {@code paid} if the transaction goes through, or to {@code failed} or {@code canceled} (within 5 business days). Some failed payouts may initially show as {@code paid} but then change to {@code failed}.
   */
  ...
  /**
   * <p>To send funds to your own bank account, you create a new payout object. Your <a href="https://stripe.com/docs/api#balance">Stripe balance</a> must be able to cover the payout amount, or you’ll receive an “Insufficient Funds” error.</p>
   *
   * <p>If your API key is in test mode, money won’t actually be sent, though everything else will occur as if in live mode.</p>
   *
   * <p>If you are creating a manual payout on a Stripe account that uses multiple payment source types, you’ll need to specify the source type balance that the payout should draw from. The <a href="https://stripe.com/docs/api#balance_object">balance object</a> details available and pending amounts by source type.</p>
   */
