...
public class ReportRun extends ApiResource implements HasId {
  ...
  /**
   * If something should go wrong during the run, a message about the failure (populated when {@code status=failed}).
   */
  ...
  /**
   * {@code true} if the report is run on live mode data and {@code false} if it is run on test mode data.
   */
  ...
  /**
   * The ID of the <a href="https://stripe.com/docs/reports/report-types">report type</a> to run, such as {@code "balance.summary.1"}.
   */
  @SerializedName("report_type")
  String reportType;

  /**
   * The file object representing the result of the report run (populated when {@code status=succeeded}).
   */
  @SerializedName("result")
  File result;

  /**
   * Status of this report run. This will be {@code pending} when the run is initially created. When the run finishes, this will be set to {@code succeeded} and the {@code result} field will be populated. Rarely, we may encounter an error, at which point this will be set to {@code failed} and the {@code error} field will be populated.
   */
  @SerializedName("status")
  String status;

  /**
   * Timestamp at which this run successfully finished (populated when {@code status=succeeded}). Measured in seconds since the Unix epoch.
   */
