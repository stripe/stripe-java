...
public class Discount extends StripeObject implements HasId {
  ...
  /**
   * If the coupon has a duration of {@code repeating}, the date that this discount will end. If the coupon has a duration of {@code once} or {@code forever}, this attribute will be null.
   */
  @SerializedName("end")
  Long end;

  /**
   * The ID of the discount object. Discounts cannot be fetched by ID. Use {@code expand[]=discounts} in API calls to expand discount IDs in an array.
   */
  ...
  /**
   * The invoice item {@code id} (or invoice line item {@code id} for invoice line items of type='subscription') that the discount's coupon was applied to, if it was applied directly to a particular invoice item or invoice line item.
   */
