...
public class RecipientUpdateParams extends ApiRequestParams {
  ...
  /**
   * A U.S. Visa or MasterCard debit card (not prepaid) to attach to the recipient. You can provide either a token, like the ones returned by <a href="https://stripe.com/docs/js">Stripe.js</a>, or a dictionary containing a user's debit card details, with the options described below. Passing {@code card} will create a new card, make it the new recipient default card, and delete the old recipient default (if one exists). If you want to add additional debit cards instead of replacing the existing default, use the <a href="https://stripe.com/docs/api#create_card">card creation API</a>. Whenever you attach a card to a recipient, Stripe will automatically validate the debit card.
   */
  @SerializedName("card")
  Object card;
