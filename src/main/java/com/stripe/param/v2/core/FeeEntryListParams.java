// File generated from our OpenAPI spec
package com.stripe.param.v2.core;

import com.google.gson.annotations.SerializedName;
import com.stripe.net.ApiRequestParams;
import java.time.Instant;
import java.util.HashMap;
import java.util.Map;
import lombok.EqualsAndHashCode;
import lombok.Getter;

@Getter
@EqualsAndHashCode(callSuper = false)
public class FeeEntryListParams extends ApiRequestParams {
  /** Filter by money movement id (e.g. txn_xxx, bt_xxx). */
  @SerializedName("collection_record")
  String collectionRecord;

  /** Filter for FeeEntries created at the exact specified timestamp. */
  @SerializedName("created")
  Instant created;

  /** Filter for FeeEntries created after the specified timestamp (exclusive). */
  @SerializedName("created_gt")
  Instant createdGt;

  /** Filter for FeeEntries created at or after the specified timestamp (inclusive). */
  @SerializedName("created_gte")
  Instant createdGte;

  /** Filter for FeeEntries created before the specified timestamp (exclusive). */
  @SerializedName("created_lt")
  Instant createdLt;

  /** Filter for FeeEntries created at or before the specified timestamp (inclusive). */
  @SerializedName("created_lte")
  Instant createdLte;

  /**
   * Map of extra parameters for custom features not available in this client library. The content
   * in this map is not serialized under this field's {@code @SerializedName} value. Instead, each
   * key/value pair is serialized as if the key is a root-level field (serialized) name in this
   * param object. Effectively, this map is flattened to its parent instance.
   */
  @SerializedName(ApiRequestParams.EXTRA_PARAMS_KEY)
  Map<String, Object> extraParams;

  /** Filter by fee batch id (fb_xxx). */
  @SerializedName("fee_batch")
  String feeBatch;

  /** Filter by usage object id (e.g. ch_xxx, py_xxx). */
  @SerializedName("incurred_by")
  String incurredBy;

  /** Maximum number of results to return per page. Defaults to 20. */
  @SerializedName("limit")
  Long limit;

  private FeeEntryListParams(
      String collectionRecord,
      Instant created,
      Instant createdGt,
      Instant createdGte,
      Instant createdLt,
      Instant createdLte,
      Map<String, Object> extraParams,
      String feeBatch,
      String incurredBy,
      Long limit) {
    this.collectionRecord = collectionRecord;
    this.created = created;
    this.createdGt = createdGt;
    this.createdGte = createdGte;
    this.createdLt = createdLt;
    this.createdLte = createdLte;
    this.extraParams = extraParams;
    this.feeBatch = feeBatch;
    this.incurredBy = incurredBy;
    this.limit = limit;
  }

  public static Builder builder() {
    return new Builder();
  }

  public static class Builder {
    private String collectionRecord;

    private Instant created;

    private Instant createdGt;

    private Instant createdGte;

    private Instant createdLt;

    private Instant createdLte;

    private Map<String, Object> extraParams;

    private String feeBatch;

    private String incurredBy;

    private Long limit;

    /** Finalize and obtain parameter instance from this builder. */
    public FeeEntryListParams build() {
      return new FeeEntryListParams(
          this.collectionRecord,
          this.created,
          this.createdGt,
          this.createdGte,
          this.createdLt,
          this.createdLte,
          this.extraParams,
          this.feeBatch,
          this.incurredBy,
          this.limit);
    }

    /** Filter by money movement id (e.g. txn_xxx, bt_xxx). */
    public Builder setCollectionRecord(String collectionRecord) {
      this.collectionRecord = collectionRecord;
      return this;
    }

    /** Filter for FeeEntries created at the exact specified timestamp. */
    public Builder setCreated(Instant created) {
      this.created = created;
      return this;
    }

    /** Filter for FeeEntries created after the specified timestamp (exclusive). */
    public Builder setCreatedGt(Instant createdGt) {
      this.createdGt = createdGt;
      return this;
    }

    /** Filter for FeeEntries created at or after the specified timestamp (inclusive). */
    public Builder setCreatedGte(Instant createdGte) {
      this.createdGte = createdGte;
      return this;
    }

    /** Filter for FeeEntries created before the specified timestamp (exclusive). */
    public Builder setCreatedLt(Instant createdLt) {
      this.createdLt = createdLt;
      return this;
    }

    /** Filter for FeeEntries created at or before the specified timestamp (inclusive). */
    public Builder setCreatedLte(Instant createdLte) {
      this.createdLte = createdLte;
      return this;
    }

    /**
     * Add a key/value pair to `extraParams` map. A map is initialized for the first `put/putAll`
     * call, and subsequent calls add additional key/value pairs to the original map. See {@link
     * FeeEntryListParams#extraParams} for the field documentation.
     */
    public Builder putExtraParam(String key, Object value) {
      if (this.extraParams == null) {
        this.extraParams = new HashMap<>();
      }
      this.extraParams.put(key, value);
      return this;
    }

    /**
     * Add all map key/value pairs to `extraParams` map. A map is initialized for the first
     * `put/putAll` call, and subsequent calls add additional key/value pairs to the original map.
     * See {@link FeeEntryListParams#extraParams} for the field documentation.
     */
    public Builder putAllExtraParam(Map<String, Object> map) {
      if (this.extraParams == null) {
        this.extraParams = new HashMap<>();
      }
      this.extraParams.putAll(map);
      return this;
    }

    /** Filter by fee batch id (fb_xxx). */
    public Builder setFeeBatch(String feeBatch) {
      this.feeBatch = feeBatch;
      return this;
    }

    /** Filter by usage object id (e.g. ch_xxx, py_xxx). */
    public Builder setIncurredBy(String incurredBy) {
      this.incurredBy = incurredBy;
      return this;
    }

    /** Maximum number of results to return per page. Defaults to 20. */
    public Builder setLimit(Long limit) {
      this.limit = limit;
      return this;
    }
  }
}
