...
public class SubscriptionCreateParams extends ApiRequestParams {
  ...
  /**
   * A future timestamp to anchor the subscription's <a href="https://stripe.com/docs/subscriptions/billing-cycle">billing cycle</a>. This is used to determine the date of the first full invoice, and, for plans with {@code month} or {@code year} intervals, the day of the month for subsequent invoices.
   */
  ...
  /**
   * Determines how to handle <a href="https://stripe.com/docs/subscriptions/billing-cycle#prorations">prorations</a> resulting from the {@code billing_cycle_anchor}. Valid values are {@code create_prorations} or {@code none}.
   *
   * <p>Passing {@code create_prorations} will cause proration invoice items to be created when applicable. Prorations can be disabled by passing {@code none}. If no value is passed, the default is {@code create_prorations}.
   */
